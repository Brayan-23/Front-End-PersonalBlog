...
function Perfil() {
  ...
  return (
    <div className='container mx-auto mt-4 rounded-2xl overflow-hidden'>
      ...
      <img src={usuario.photo} alt={`Foto de perfil de ${usuario.name}`} className='rounded-full w-56 mx-auto mt-[-8rem] border-8 border-white relative z-10' />
      <div className="relative mt-[-6rem] h-72 flex flex-col bg-sky-500 text-white text-2xl items-center justify-center">
        <p>Nome: {usuario.name} </p>
        <p>Email: {usuario.usr}</p>
      </div>
    </div>
  )
}
